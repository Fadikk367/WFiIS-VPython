...
def calc_energy():
  E = g*ballA.pos.y + g*ballB.pos.y  + (1/2)*L**2 * (ballA.phi_prim**2 + ballA.phi_prim**2 + ballB.theta_prim**2 + 2*ballA.phi_prim*ballB.theta_prim*cos(ballA.phi - ballB.theta))
  return E

def update_balls(ballA, ballB, dt):
  phi_bis = ((-g/ballA.L)*(2*sin(ballA.phi) - sin(ballB.theta)*cos(ballA.phi - ballB.theta)) - (ballA.phi_prim**2) * sin(2*ballA.phi - 2*ballB.theta)/2 - ballB.theta_prim**2 * sin(ballA.phi - ballB.theta)) / (1 + (sin(ballA.phi - ballB.theta))**2)

  theta_bis = (-(g/ballA.L)*(2*sin(ballB.theta) - 2*sin(ballA.phi)*cos(ballA.phi - ballB.theta)) + (ballB.theta_prim**2) * sin(2*ballA.phi - 2*ballB.theta)/2 + 2*(ballA.phi_prim**2) * sin(ballA.phi - ballB.theta)) / (1 + (sin(ballA.phi - ballB.theta))**2)
